...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $image = $_POST['image'];
    $updateStmt = $mysql->prepare("UPDATE  `notable alumni` SET  firstname = ?, lastname = ?, patr = ?, biography=?, image = ? WHERE id = ?");
    $updateStmt->bind_param("sssssi", $firstname, $lastname, $patr, $biography,$image, $alumniID);
    $updateStmt->execute();
    $updateStmt->close();
...
$alumniID = $_GET['id'];
$selectStmt = $mysql->prepare("SELECT * FROM `notable alumni` WHERE id = ?");
$selectStmt->bind_param("i", $alumniID);
$selectStmt->execute();
...
<body>
    ...
    ?>
    <div class="container">
        <h2 style="text-align:center;font-weight:300">Редактирование выпускника</h2>
